$(document).ready( function(){
    ...
    var viewport_w = parseInt(mbt_au_frontend_object.mbt_ua_setting.file_dimension.w) || 100;
    var viewport_h = parseInt(mbt_au_frontend_object.mbt_ua_setting.file_dimension.h) || 100;
    
    var boundary_w = parseInt(viewport_w) + 100;
    var boundary_h = parseInt(viewport_h) + 100;
    ...
    $('#mbt_avatar').on('change', function () {
        ...
        var avatar_size = (this.files[0].size / 1024); 
        var avatar_size_kb = (Math.round(avatar_size * 100) / 100);
        
        //var fileExtension = ['jpeg', 'jpg', 'png'];
        var obj = mbt_au_frontend_object.mbt_ua_setting.file_type;
        var fileExtension = Object.keys(obj).map(function (key) { 
            return obj[key]; 
        });
        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
            ...
            var max_size_mb = (Math.round(mbt_au_frontend_object.mbt_ua_setting.max_size / 1024));
            if(max_size_mb < 1) {
